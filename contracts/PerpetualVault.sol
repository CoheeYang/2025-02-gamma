...
import {ParaSwapUtils} from "./libraries/ParaSwapUtils.sol";
import {Error} from "./libraries/Error.sol";
import {IDataStore} from "./interfaces/gmx/IDataStore.sol";
import {IExchangeRouter} from "./interfaces/gmx/IExchangeRouter.sol";
import {IPerpetualVault} from "./interfaces/IPerpetualVault.sol";
import {IGmxProxy} from "./interfaces/IGmxProxy.sol";
import {IVaultReader} from "./interfaces/IVaultReader.sol";
import {MarketPrices, MarketProps} from "./libraries/StructData.sol";
...
contract PerpetualVault is IPerpetualVault, Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  */
  function deposit(uint256 amount) external nonReentrant payable {
    ...
    EnumerableSet.add(userDeposits[msg.sender], counter);//@audit Q 不明白这个是干嘛的
  ...
  */
  function run(//@audit Q:似乎这个函数自带MEV问题，
    bool isOpen,
    bool isLong,
    MarketPrices memory prices,
    ...
    if (isOpen) {//开户
      if (positionIsClosed) {//判断现在有没有户
        if (_isFundIdle() == false) {//没有户，Checks if the contract has idle collateral funds that meet the minimum deposit amount.
          revert Error.InsufficientAmount();
        }
        if (_isLongOneLeverage(isLong)) {
          _runSwap(metadata, true, prices);//@audit Q: 为什么在没有户要开户的时候输入了true,而情况其他开户都是false？
        } else {
          (uint256 acceptablePrice) = abi.decode(metadata[0], (uint256));
          _createIncreasePosition(isLong, acceptablePrice, prices);
        }
      } else {//目前有户的情况
        if (beenLong == isLong) {//@audit Q：这个是检验现在头寸和要开的头寸是否相等，但我没看见这个beenLong在哪更新过，这个false==flase的情况怎么办，有没有办法开户后，让beenLong为false
          revert Error.NoAction();
        } else {
          // Close current position first and then open the requested position in the next action
          nextAction.selector = NextActionSelector.INCREASE_ACTION;
          nextAction.data = abi.encode(isLong);///@audit Q:这咋关户的？
      ...
      if (positionIsClosed == false) {//有没有户
        if (_isLongOneLeverage(beenLong)) {//有户，且是1倍杠杆多头
          _runSwap(metadata, false, prices);
        } else {//有户，但不是一杯杠杆多头
          (uint256 acceptablePrice) = abi.decode(metadata[0], (uint256));
          _createDecreasePosition(0, 0, beenLong, acceptablePrice, prices);
  ...
  function runNextAction(MarketPrices memory prices, bytes[] memory metadata) external nonReentrant gmxLock {
    _onlyKeeper();
    Action memory _nextAction = nextAction;//@audit Q:会有被不同的执行情况cover的情况吗？
    delete nextAction;
  ...
  */
  ...
  /* @audit 值得一看，这个是gmxProxy执行的逻辑，是否有可能拿gmx在错误的时间传入---
  ...
  function afterLiquidationExecution() external {
    ...
    //@audit Q:没有改变? _gmxLock = false;也没有
    depositPaused = true;
  ...
  */
  ...
  /* @audit Q:给钱不找零？会打乱后续share的分配数量吗？
   * @notice
  ...
  */
  function _runSwap(bytes[] memory metadata, bool isCollateralToIndex, MarketPrices memory prices) internal returns (bool completed) {
    ...
    }//判断metadata长度是1还是2，其他的长度都会被revert
    if (metadata.length == 2) {//如果数据长度为2则:1.解码判断dex，
                                //2.用deDexSwap()，更新swapProgressData.swapped数据
                                //3.解码数列元素的第二bytes数据，判断GMX
                                //4.用_doGmxSwap()，返回false
      (PROTOCOL _protocol, bytes memory data) = abi.decode(metadata[0], (PROTOCOL, bytes));
      if (_protocol != PROTOCOL.DEX) {//1.
        revert Error.InvalidData();
      }
      swapProgressData.swapped = swapProgressData.swapped + _doDexSwap(data, isCollateralToIndex);//2.
      
      (_protocol, data) = abi.decode(metadata[1], (PROTOCOL, bytes));
      if (_protocol != PROTOCOL.GMX) {//3.
        revert Error.InvalidData();
      }
      ...
      if (_protocol == PROTOCOL.DEX) {
        ...
        return true;
      } else {//@audit Q:这里逻辑有问题吧，解析出来要是地址要是传个别的东西咋办，影响不大但是逻辑不完整，metadata.length==1，且protocol!=DEX（0）的数据都会造成错乱
        _doGmxSwap(data, isCollateralToIndex);
        return false;
      }
    }
  ...
  */
  ...
  function _handleReturn(uint256 withdrawn, bool positionClosed, bool refundFee) internal {//handleReturn(0,true,flase)
    (uint256 depositId) = flowData;
    uint256 shares = depositInfo[depositId].shares;
    uint256 amount;
    if (positionClosed) {
      amount = collateralToken.balanceOf(address(this)) * shares / totalShares;///withdraw金额等于合约余额x用户对应的share/总share@audit Q: 这个share会不会被曲解？
    } else {
    ...
    if (refundFee) {
      ...
      if (depositInfo[depositId].executionFee > usedFee) {
        try IGmxProxy(gmxProxy).refundExecutionFee(depositInfo[counter].owner, depositInfo[counter].executionFee - usedFee) {} catch {}//@audit Q USDC blacklist
      }
    }
  ...
  function _transferToken(uint256 depositId, uint256 amount) internal {
    uint256 fee;
    //@audit Q: 为什么会有withdraw amount 大于deposit amount的情况？-->profit情况
    ...
    // @audit Q if the ERC20 does not revert, it would never catch the next line
    // 它们有没有检查amount-fee <= 0的情况
  ...
  function _updateState(
    bool _positionIsClosed,//false true
    bool _isLong
  ) internal {
    if (flow == FLOW.SIGNAL_CHANGE) {
      positionIsClosed = _positionIsClosed;
  ...
  */
  ...
  function _onlyKeeper() internal view {
    if (msg.sender != keeper) revert Error.NotAKeeper();
  }
